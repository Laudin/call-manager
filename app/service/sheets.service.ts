...
class SheetsService {
  ...
  // google.options({ auth: credentials });
    this.SPREADSHEET_ID = SPREADSHEET_ID || '1yRT2DecwyA_IcEsuoIgaJP4ztrB77ngeg7NPBaxeUAE'
  }

  async get(range: string = 'A:H'): Promise<{ok: true, data: any[][]} | {ok: false, data: Error}> {
    try {
